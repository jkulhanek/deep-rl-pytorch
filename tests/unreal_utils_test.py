...
import numpy as np
import torch
from deep_rl.actor_critic.unreal.utils import autocrop_observations, pixel_control_reward, pixel_control_loss, value_loss, reward_prediction_loss
...
class UtilTest(unittest.TestCase):
    ...
    def testPixelControlRewardValues(self):
        ...
        pseudo_rewards = pixel_control_reward(observations, output_size=(20, 20))
        last_rewards = torch.from_numpy(start.astype(np.float32)).unsqueeze(0).unsqueeze(1)
    ...
    def testPixelControlLoss(self):
        ...
        action_predictions = torch.rand((10, 5, 4, 20, 20), dtype=torch.float32)
