def _merge_value(original_value, value):
    ...
    if isinstance(original_value, (dict, Configuration)):
        for key, val in value.items():
            original_value[key] = _merge_value(original_value.get(key, None), val)
...
class Configuration:
    ...
    def _merge_value(self, original_value, value):
        ...
        if isinstance(original_value, (dict, Configuration)):
            for key, val in value.items():
                original_value[key] = self._merge_value(original_value.get(key, None), val)
...
_config_attrs = ['get', '__getattribute__', '__setattr__', '__str__', '__repr__', '__iter__', 'data', 'as_dict', 'update']
class ConfigWrapper:
