...
class Trainer(object):
    ...
    def _process_pc(self, sess):
        ...
        for i, frame in enumerate(pc_experience_frames[1:]):
            pixel_change = self._calc_pixel_change(pc_experience_frames[i].state[0], frame.state[0])
            pc_R = pixel_change + self.gamma_pc * pc_R
            a = np.zeros([self.action_size])
            a[frame.action] = 1.0
            last_action_reward = frame.state[1]

            batch_pc_si.append(frame.state[0])
            batch_pc_a.append(a)
    ...
    , sess):
        ...
        for frame in vr_experience_frames[1:]:
            ...
            batch_vr_si.append(frame.state[0])
            batch_vr_R.append(vr_R)
            batch_vr_last_action_reward.append(frame.state[1])
    ...
    def _process_rp(self):
        ...
        for i in range(3):
            batch_rp_si.append(rp_experience_frames[i].state[0])
