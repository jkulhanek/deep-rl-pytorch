...
def build_unreal(BaseClass, name=None,
    ...
    def __init__(self, model_fn, env_fn, *args,
        ...
        BaseClass.__init__(self, model_fn, env_fn, *args, num_steps=num_steps, **kwargs)
    ...
    def compute_loss(self, batch: UnrealBatch):
        loss, losses = BaseClass.compute_loss(self, batch.base)
    ...
    def sample_experience_batch(self):
        batch = BaseClass.sample_experience_batch(self)
    ...
    def store_experience(self, observations, actions, rewards, terminals, values, action_log_prob, states):
        ...
        BaseClass.store_experience(self, observations, actions, rewards, terminals, values, action_log_prob, states)

    def collect_experience(self):
        if not self.replay.full:
            while not self.replay.full:
                result = BaseClass.collect_experience(self)
    ...
    def state_dict(self):
        state_dict = BaseClass.state_dict(self)
        del state_dict['replay']
        return state_dict

    class_type = type(name or f'Unreal{BaseClass.__name__}', (BaseClass,), locals())
